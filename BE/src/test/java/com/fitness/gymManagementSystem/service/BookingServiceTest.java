...
class BookingServiceTest {
    ...
    private User mockTrainer; // 🔥 THÊM MOCK TRAINER ĐỂ FIX LỖI NULL
    ...
    void setUp() {
        ...
        mockTrainer.setUsername("trainer");
        ...
        mockClass.setTrainer(mockTrainer); // 🔥 GẮN TRAINER VÀO LỚP ĐỂ KHÔNG BỊ NPE
    }
    ...
    // --- BỔ SUNG ĐỂ ĐẠT 100% BOOKING SERVICE ---
    
    @Test
    void getMembersByClass_NotTrainer_ThrowsException() {
        when(gymClassService.getEntityById(10L)).thenReturn(mockClass); // 🔥 Đổi về 10L cho đồng bộ
        
        // Người gọi API là "wrong_trainer" nhưng chủ lớp là "trainer"
        IllegalStateException ex = assertThrows(IllegalStateException.class, 
                () -> bookingService.getMembersByClass(10L, "wrong_trainer"));
        assertEquals("Bạn không phải trainer của lớp này", ex.getMessage());
    }

    @Test
    void getMembersByClass_Success() {
        when(gymClassService.getEntityById(10L)).thenReturn(mockClass); // 🔥 Đổi về 10L cho đồng bộ
        
        ClassBooking booking = new ClassBooking();
        booking.setUser(mockUser);
        
        when(bookingRepository.findByGymClass_Id(10L)).thenReturn(List.of(booking)); // Đổi về 10L

        List<UserResponse> res = bookingService.getMembersByClass(10L, "trainer"); // Truyền đúng tên trainer
        assertFalse(res.isEmpty());
        assertEquals("member123", res.get(0).username()); // 🔥 Đổi về member123 cho khớp dữ liệu setup
        }
